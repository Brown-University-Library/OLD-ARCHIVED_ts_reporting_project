...
def bul_login(func):
    ...
    def decorator(request, *args, **kwargs):
        ...
        shib_dict = hlpr.prep_shib_dct( request )
        user_obj = hlpr.manage_usr_obj( request, shib_dict )
        if not user_obj:
...
class LoginDecoratorHelper(object):
    ...
    def prep_shib_dct( self, request ):
    ...
    def manage_usr_obj( self, request, data ):
        """ Pull information for the Shib request and get/create and login Django User object.
            Called by bul_login() """
        ( username, netid ) = self.set_basics( request.get_host(), data )
        if not username or not netid:
            return
        u = self.update_userobj( username, netid )
    ...
    def set_basics( self, host, request_dct ):
